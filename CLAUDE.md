...
- REASONING (absorb): Discord has no known error that classifies as `'absorb'` — editing to identical content is a no-op success, not an error. The branch is retained in the union type for parity with Telegram's classifier so downstream callers can use a single `switch (result.kind)` pattern regardless of bridge. In practice, `classifyDiscordError` never returns `{kind:'absorb'}`. **Test #23 enforces this invariant** by iterating every fixture in `__fixtures__/discord-errors.json` and asserting `kind !== 'absorb'`. New fixture additions can't silently start exercising the absorb path. **Invariant-violation handling:** if the invariant ever breaks, `#sendFn` throws a clear `[DiscordEditStreamingAdapter] sendFn hit absorb branch — DECISION 10-F invariant violated` error (PR #10 review round 1) — returning `''` would hand the session a bogus message id and a subsequent `editFn('', text)` would paper over the failure. `#editFn`'s absorb branch returns silently (matches Telegram's absorb-as-success semantics for editFn-shaped callsites); either behavior is defensible but failing loud on `#sendFn` catches the invariant break at the earliest observable moment.
